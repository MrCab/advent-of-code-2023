...
class OasisSolver :
  ...
  def processAllLines( self ) :
    ...
    for line in self.allLines :
      ...
      ##part 1
      ##sumTotal += nums[-1] + self.mathALine( nodes )[1]

      ## part 2
      sumTotal += nums[0] - self.mathALine( nodes )[0]
    print( sumTotal )
  ...
  def mathALine( self, oNodes ) :
    ...
    if len( vals ) == 1 :
      returnMe = ( list(vals)[0], list(vals)[0] )
    # Else, recurse, and add the value to the last node
    else:
      diff = self.mathALine( childONodes )
      returnMe = ( childONodes[0].value - diff[0], childONodes[-1].value + diff[1] )
    return returnMe
