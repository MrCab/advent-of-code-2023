...
class SpiritTempleSolver :
  ...
  def findSpiritTemple( self, startNode, endRegex ) :
    step = 0
    curNode = startNode
    i = 0

    while not re.match( endRegex, curNode ) :
  ...
  def main(self) :
    ...
    #p1 = self.findSpiritTemple("AAA", "ZZZ")

    aButtons = [ a for a in list( self.nodes.keys() ) if a[-1] == "A" ]
    zTriggers = [ self.findSpiritTemple( z, "..Z" ) for z in aButtons ]
    print( "LCM IS" )
    print (zTriggers)
    print( "LCM IS" )
    print ( math.lcm( *zTriggers ) )
