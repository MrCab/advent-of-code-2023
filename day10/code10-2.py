...
#######
  def callingThisGoUntilRealPlayersGetMad( self ) :
    territory = 0
    for line in self.distancesFromStart :
      inny = 0
      i = 0
      while i < len(line):
        # if it's not a pipe in the loop
        if line[i] is None :
          territory += inny % 2
        # if it is a pipe, flip the "inside" bit
        else :
          inny += 1
        i += 1
    print ( "Territory: %d" % ( territory ) )
  ...
  def main(self) :
    ...
    self.callingThisGoUntilRealPlayersGetMad()
