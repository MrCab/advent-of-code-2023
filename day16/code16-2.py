...
#      return
  ...
  def main(self) :
    ...
    maxX = len(self.allLines[0])
    maxY = len(self.allLines)
    i = 0
    while i < maxY :
      allCalls.append("self.goToTheLight(0,%d, 'R')" % (i) )
      allCalls.append("self.goToTheLight(%d,%d, 'L')" % (maxX-1,i) )
      i += 1
    i = 0
    while i < maxX :
      allCalls.append("self.goToTheLight(%d,0, 'D')" % (i) )
      allCalls.append("self.goToTheLight(%d,%d, 'U')" % (i,maxY-1) )
      i += 1

    for starterString in allCalls :
#      starterString = "self.goToTheLight(0,0, 'R')"
      self.discordWillHateMe.append( starterString )
      self.reset()
      while len( self.discordWillHateMe ) > 0 :
        foo = self.discordWillHateMe.pop(0)
        exec( foo )
      thisCount = self.countOfEnergized()
      if thisCount > maxCount :
        maxCount = thisCount
        maxCall = starterString
        print( starterString + " = " + str(maxCount) )
